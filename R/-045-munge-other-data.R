...
basil_df <- basil_redcap %>%
  ...
  select(studyid, vdsite,  vdage, vdfemale, vdeduc, vdhisp, vdesl, vdvisionimp, vdhearingimp,
          vdcharlson, vdmajneurogs, vdminneurogs, vddeliriumgs, vdcamsrs,
          phq_9_score, moca_score, ad8_total_score) %>%
  group_by(studyid) %>%
  fill(vdcamsrs, vdage, vdfemale, vdhisp,
       vdeduc, vdcharlson, vdmajneurogs, vdminneurogs, vddeliriumgs, vdsite, vdesl, vdhearingimp,
       vdvisionimp, phq_9_score, moca_score, ad8_total_score,
       .direction = "downup") %>%
  slice(1) %>%
  ungroup()
...
# labelled::var_label(basil_df$vddelseveritygs) <- "Delirium severity (GS) continuous"
# labelled::var_label(basil_df$vddemseveritygs) <- "Dementia severity (GS) continuous"
