...
ba_initial_wt <- ba_a1_weighted %>%
    ...
    scale_x_continuous("Delirium severity: Rater A & Rater B Average", 
                       limits = c(0, 10), breaks = seq(0, 10, 2)) +
    scale_y_continuous("Delirium severity: Rater A & Rater B Difference", 
                       limits = c(-5, 5), breaks = seq(-6, 6, 2)) +
    hrbrthemes::theme_ipsum()
```

```{r}
ggsave(here::here("Figures", "blandaltman_inital_wt.pdf"), plot = ba_initial_wt, 
...
```{r}
ggsave(here::here("Figures", "blandaltman_consensus_wt.pdf"), plot = ba_iconsensus_wt, 
       device = cairo_pdf, dpi = 300,
       units = "in", height = 4, width = 6)
