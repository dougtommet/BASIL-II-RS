...
basil_foo <- basil_df %>%
  ...
  select(-vd_clinical_assessor, -vdsite2, -vdsite3, -w_eb, -w_lr) %>%
  select(
     vdsite,          vdage,           vdfemale,       vdnonwhite,     vdblack,  vdrace,      vdhisp,         vdesl,          vdeduc,         vdlivesalone,  
     vdmarried,       vdlivesinnh,     vdvisionimp,    vdhearingimp,   vdcharlson,     vdcci3,         vdmajneurogs,   vdminneurogs,   vddeliriumgs,   
...
basil_foo %>%
  mutate(vddelseveritygs_cont = vddelseveritygs) %>%
  relocate(vddelseveritygs_cont, .after = vddelseveritygs) %>%
  haven::zap_labels(basil_foo$vddelseveritygs_cont) %>%
  sjlabelled::var_labels(vddelseveritygs = "Delirium severity (GS) (categorical)",
                         vddelseveritygs_cont = "Delirium severity (GS) (continuous)") %>%
  labelled::to_factor() %>%
  
  gtsummary::tbl_summary(by = vdsite,
                         type = list(c(vdeduc, vddelseveritygs_cont) ~ "continuous",
...
basil_foo <- basil_df %>%
  ...
  select(-vd_clinical_assessor, -vdsite2, -vdsite3, -w_lr) %>%
  select(
     vdsite,          vdage,           vdfemale,       vdnonwhite,     vdblack,   vdrace,     vdhisp,         vdesl,          vdeduc,         vdlivesalone,  
     vdmarried,       vdlivesinnh,     vdvisionimp,    vdhearingimp,   vdcharlson,     vdcci3,         vdmajneurogs,   vdminneurogs,   vddeliriumgs,   
     vddelseveritygs, vddemseveritygs, vdcamsrs,       phq_9_score,    moca_score,     ad8_total_score, w_eb
  )
